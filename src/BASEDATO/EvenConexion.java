...
//    DefaultListModel model_lista;
    ...
    public int getInt_SumarOrden_mas_uno(Connection conn,String tabla,String orden,String idcampo,int iddato){
        String titulo="getInt_ultimoID";
        int getid=0;
        String sql="select max("+orden+"+1) as orden from "+tabla+" where "+idcampo+"="+iddato+";";
        try {
            ResultSet rs=getResulsetSQL(conn, sql, titulo);
            if(rs.next()){
                getid=rs.getInt("orden");
                if(getid==0){
                    getid=1;
                }
            }
    ...
    public int getInt_seleccionar_JLista(Connection conn, JTextField txtbuscar, JList jLista,boolean visible, 
            String tabla, String campoNombre,String campoMostrar,String campoCargar, String campoid) {
        ...
        if (fila_ruc >= 0) {
            ...
            try {
                ...
                if (rs.next()) {
                    ...
                    txtbuscar.setText(rs.getString(campoCargar));
    ...
    public int getInt_filtro_sql(Connection conn,String sql){
        String titulo="getInt_filtro_sql";
        int getcampo=0;        
        try {
            ResultSet rs = getResulsetSQL(conn,sql,titulo);
            if (rs.next()) {
                getcampo = rs.getInt(1);
            }
        } catch (Exception e) {
            evmen.mensaje_error(e, sql,titulo);
        }
