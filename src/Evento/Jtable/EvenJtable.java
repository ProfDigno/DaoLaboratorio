    ...
    public String getString_select(JTable tabla, int columna) {
        ...
        if (row >= 0) {
            try {
                ...
                System.err.println("getString_select:"+e);
                sid = "0";
            }
...
//            JOptionPane.showMessageDialog(tabla, "SELECCIONAR UNA FILA PARA ELIMINAR");
    ...
    public void seleccionar_tabla_flecha_abajo(java.awt.event.KeyEvent evt,JTable tabla){
        if (evt.getKeyCode() == KeyEvent.VK_DOWN) {
            tabla.requestFocus();
            tabla.changeSelection(0, 0, false, false);
        }
    }
    public void saltar_a_campo_enter(java.awt.event.KeyEvent evt,JTable tabla,JTextField txttexto){
        if (evt.getKeyCode() == KeyEvent.VK_ENTER) {
            int fila = tabla.getSelectedRow();
            tabla.requestFocus();
            tabla.changeSelection(fila-1, 0, false, false);
