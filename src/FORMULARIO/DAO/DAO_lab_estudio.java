...
public class DAO_lab_estudio {
    ...
    private String sql_insert = "INSERT INTO public.lab_estudio(idlab_estudio,nombre_completo,nombre_corto,numerico_decimal,"
            + "es_numerico,es_testo,es_predefinido,es_validado,"
            + "valor_de_referencia,fk_idlab_unidad,fk_idlab_grupo_estudio,sku,se_carga,concepto_id,es_estudio) VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?);";
    private String sql_update = "UPDATE public.lab_estudio SET nombre_completo=?,nombre_corto=?,numerico_decimal=?,"
            + "es_numerico=?,es_testo=?,es_predefinido=?,es_validado=?,"
            + "valor_de_referencia=?,fk_idlab_unidad=?,fk_idlab_grupo_estudio=?,sku=?,se_carga=?,concepto_id=?,es_estudio=? WHERE idlab_estudio=?;";
    private String sql_cargar = "SELECT idlab_estudio,nombre_completo,nombre_corto,numerico_decimal,"
            + "es_numerico,es_testo,es_predefinido,es_validado,"
            + "valor_de_referencia,fk_idlab_unidad,fk_idlab_grupo_estudio,sku,se_carga,concepto_id,es_estudio FROM public.lab_estudio WHERE idlab_estudio=";
    ...
    public void insertar_lab_estudio(Connection conn, lab_estudio lab_es) {
        ...
        try {
            ...
            pst.setInt(12, lab_es.getC12sku());
            pst.setBoolean(13, lab_es.isC13se_carga());
            pst.setInt(14, lab_es.getC14concepto_id());
            pst.setBoolean(15, lab_es.isC15es_estudio());
            pst.execute();
            pst.close();
    ...
    public void update_lab_estudio(Connection conn, lab_estudio lab_es) {
        ...
        try {
            ...
            pst.setInt(11, lab_es.getC12sku());
            pst.setBoolean(12, lab_es.isC13se_carga());
            pst.setInt(13, lab_es.getC14concepto_id());
            pst.setBoolean(14, lab_es.isC15es_estudio());
            pst.setInt(15, lab_es.getC1idlab_estudio());
    ...
    public void cargar_lab_estudio(Connection conn, lab_estudio lab_es, int id) {
        ...
        try {
            ...
            if (rs.next()) {
                ...
                lab_es.setC12sku(rs.getInt(12));
                lab_es.setC13se_carga(rs.getBoolean(13));
                lab_es.setC14concepto_id(rs.getInt(14));
                lab_es.setC15es_estudio(rs.getBoolean(15));
    ...
    public void actualizar_tabla_lab_estudio(Connection conn, JTable tbltabla,String filtro) {
        String sql_select_buscar = "select les.idlab_estudio as idest,\n"
            ...
            + "and lge.fk_idlab_grupo="+tbl_or.getIdtabla_grupo_valor()+"\n"
                + "and ile1.fk_idlab_estudio=les.idlab_estudio)\n"
            + "else 0 end as orden,\n"
            + "les.nombre_completo as nom_completo,les.sku,les.concepto_id,\n"
            + "lun.nombre as unid \n"
            + "from public.lab_estudio les,public.lab_unidad lun\n"
            + "where les.fk_idlab_unidad=lun.idlab_unidad\n"+filtro
            + " order by 2 asc,3 desc,4 desc,5 asc,6 asc;";
    ...
    public void ancho_tabla_lab_estudio(JTable tbltabla) {
        int Ancho[] = {5,15,18,18,5,24,5,5,5};
